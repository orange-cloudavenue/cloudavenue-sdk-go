...
func (e *EdgeClient) GetALLALBServiceEngineGroups() ([]*EdgeGatewayALBServiceEngineGroupModel, error) {
	// Check if the edge name or ID is empty
	if e.vcdEdge.EdgeGateway.ID == "" || e.EdgeName == "" {
		return nil, fmt.Errorf("edge name or ID is empty, please provide a valid name or id")
	}
...
func (e *EdgeClient) GetALBServiceEngineGroup(nameOrID string) (*EdgeGatewayALBServiceEngineGroupModel, error) {
	...
	if urn.IsServiceEngineGroup(nameOrID) {
		...
		if len(govcdSEGs) > 1 {
			return nil, fmt.Errorf("more than one service engine group found for edge gateway %s", e.EdgeName)
		}
